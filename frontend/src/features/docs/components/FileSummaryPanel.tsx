...
export function FileSummaryPanel({ docData }: FileSummaryPanelProps) {
    ...
    const selectFolder = (g: FolderGroup) => {
        setSelection({ type: "folder", path: g.path, summary: g.summary });
        if (!openFolders.has(g.path)) {
            setOpenFolders((prev) => new Set(prev).add(g.path));
        }
    };

    const selectFile = (item: DocFileSummaryItem) => {
