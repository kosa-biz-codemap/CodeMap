...
## 5A. PROJECT-REPO-FILE-API-001 (분석 파일 컨텐츠 조회)
`GET /api/repo/analysis/{job_id}/files/content`

| HTTP Status | Error Code | 발생 시점 | 설명 |
| :--- | :--- | :--- | :--- |
| 403 | `FILE_PATH_FORBIDDEN` | 파일 경로 해결 | 요청한 상대 경로가 분석 workspace 밖을 가리키거나 path traversal 세그먼트를 포함함 |
| 404 | `WORKSPACE_NOT_READY` | workspace/파일 검증 | clone workspace가 아직 준비되지 않았거나 요청한 파일이 workspace 안에 없음 |
| 422 | `BINARY_FILE` | 파일 확장자 검증 | 미리보기 대상이 이미지, 압축 파일, 실행 파일 등 텍스트 프리뷰 불가 파일임 |
