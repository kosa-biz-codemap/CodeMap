...
API별 오류 코드는 `docs/03_Specifications/ERROR_CODES.md`를 기준으로 합니다.

## 1.1 Frontend 오류 표시 원칙

Issue #174, #176, #179, #180에 따라 frontend는 서버 오류 envelope를 그대로 문자열화하지 않습니다. 각 API client는 공통 `parseApiError` 규칙으로 오류를 정규화한 뒤 UI 계층에 전달합니다.
...
- 사용자에게 노출되는 기본 문구는 최상위 `message` 또는 `error.code`별 사용자 문구 mapping을 우선합니다.
- `error.detail`은 개발/로그/telemetry 보조 정보이며, 객체 detail을 UI에 직접 렌더링하지 않습니다.
- `error.field`가 있으면 form field 아래에 표시하고, 없으면 form/page 상단 banner 또는 toast로 표시합니다.
- `window.alert`/`window.confirm`은 제품 UI 흐름에서 사용하지 않고, inline feedback, toast/banner, confirm modal로 대체합니다.
- confirm modal은 keyboard navigation, focus trap, ESC 닫기, screen reader label을 만족해야 합니다.
