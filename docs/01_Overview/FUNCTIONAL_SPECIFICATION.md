...
> [!NOTE]
> 아래 `chat/` 및 `agent_graph/` 구조는 최신 합의안을 기준으로 한 **구현 예정/권장 구조**입니다. 현재 파일이 모두 존재한다는 뜻이 아니라, 이후 agent 기능 구현 시 맞춰갈 목표 구조입니다.
...
│   │   ├── agent_graph/      # AGENT LangGraph Layer: State, Nodes, Agents, Workers
...
| `AGENT-CHAT-B-101` | AGENT | CHAT | Backend | 멀티에이전트 채팅 실행 API | `POST /api/chat/{repo_id}/runs` 요청을 받아 chat run을 생성하고 LangGraph 실행을 시작 |
| `AGENT-CHAT-B-201` | AGENT | CHAT | Backend | chat/service 실행 관리자 | 세션 관리, LangGraph 비동기 실행 호출, run 상태 저장, SSE stream URL 반환 |
| `AGENT-CHAT-B-202` | AGENT | CHAT | Backend | Final Answer Agent | LangGraph가 반환한 State의 원본 근거와 compact_context를 기반으로 최종 답변 생성 |
| `AGENT-CHAT-B-203` | AGENT | CHAT | Backend | SSE 스트리밍 이벤트 제어 | graph_started, worker_result, answer_delta, completed, failed 등 이벤트 스트림 규격화 |
| `AGENT-CHAT-B-204` | AGENT | CHAT | Backend | run 상태 및 취소 제어 | 실행 중인 agent run의 상태 조회, timeout/cancel/failure 상태 전환 처리 |
| `AGENT-CHAT-F-201` | AGENT | CHAT | Frontend | AI 응답 UI | 답변 및 참조 파일명 표시 |
| `AGENT-CHAT-F-202` | AGENT | CHAT | Frontend | agent run 상태 표시 | Supervisor 계획, worker 실행, evidence 정리, final answer 단계 상태를 UI에 표시 |
| `AGENT-CHAT-F-203` | AGENT | CHAT | Frontend | 관련 근거 파일 표시 | State worker_results 기반 파일 경로, line range, score, tool source 표시 |
| `AGENT-CHAT-F-204` | AGENT | CHAT | Frontend | SSE 스트리밍 응답 처리 | FastAPI SSE(Server-Sent Events) 기반 graph event와 answer_delta 수신 처리 |
| `AGENT-CHAT-F-205` | AGENT | CHAT | Frontend | 답변 스트리밍 UI | LLM 답변을 실시간 스트리밍으로 받아 타이핑 효과로 표시 |
| `AGENT-CHAT-F-206` | AGENT | CHAT | Frontend | 질문 입력 및 run 생성 | 사용자 질문, 모드, includeEvidence 옵션을 API 요청으로 전달 |
| `AGENT-GRAPH-B-201` | AGENT | GRAPH | Backend | CodeMapState 스키마 정의 | user_query, rewritten_query, access_plan, security_result, worker_results, compact_context, final_answer 공유 상태 정의 |
| `AGENT-GRAPH-B-202` | AGENT | GRAPH | Backend | LangGraph workflow 정의 | Supervisor, Route Node, Workers, Evidence Aggregator node와 chat/service 반환 edge 구성 |
| `AGENT-SUPERVISOR-B-201` | AGENT | SUPERVISOR | Backend | Supervisor Agent 계획 수립 | 사용자 의도 분석, query rewrite, selected_workers, allowed_paths, risk_level plan JSON 생성 |
| `AGENT-ROUTE-B-201` | AGENT | ROUTE | Backend | Route Node plan 검증 | LLM이 아닌 코드 노드로 plan JSON schema, worker 목록, path allowlist 검증 |
| `AGENT-ROUTE-B-202` | AGENT | ROUTE | Backend | 경로 보안 및 traversal 차단 | `..`, 절대경로, `.env`, key/token 등 금지 패턴과 허용 루트 경계 검사 |
| `AGENT-ROUTE-B-203` | AGENT | ROUTE | Backend | Worker 비동기 병렬 라우팅 | 검증된 작업을 `asyncio.gather` 또는 LangGraph parallel branch로 fan-out |
| `AGENT-CORE-B-201` | AGENT | CORE | Backend | agent 시작/완료 이벤트 발행 | graph_started, supervisor_plan, route_validated, worker_started, worker_result, completed, failed 이벤트 publish |
| `AGENT-CORE-B-202` | AGENT | CORE | Backend | completed/failed 후 cleanup | final event 이후 queue 정리 |
| `AGENT-CORE-B-203` | AGENT | CORE | Backend | agent 실행 시간 측정 | 각 node/worker start/end timestamp와 latency 기록 |
| `AGENT-CORE-B-204` | AGENT | CORE | Backend | agent 실패 처리 | 실패 node/worker, error code, fallback answer context 저장 및 failed event 발행 |
...
| `AGENT-WORKER-B-201` | AGENT | WORKER | Backend | Search Worker Agent | 한국어 자연어/오타 질문을 코드 검색용 query로 확장하고 embedding/vector search 전략 실행 |
| `AGENT-WORKER-B-202` | AGENT | WORKER | Backend | Dir Worker | 허용된 repo 경로 안에서 디렉토리 구조 탐색 |
| `AGENT-WORKER-B-203` | AGENT | WORKER | Backend | Grep Worker | 키워드, 정규식, alias 기반 코드 후보 검색 |
| `AGENT-WORKER-B-204` | AGENT | WORKER | Backend | Read Worker | Route Node가 허용한 후보 파일만 읽어 raw snippet과 line range 반환 |
| `AGENT-WORKER-B-205` | AGENT | WORKER | Backend | Code Reasoning Worker | 필요 시 읽은 코드 조각을 해석하는 선택형 LLM worker |
| `AGENT-EVIDENCE-B-201` | AGENT | EVIDENCE | Backend | Evidence Aggregator Node | worker_results 중복 제거, 파일 경로/라인/근거 정리, token budget 압축 |
...
| `AGENT-MEMORY-B-201` | AGENT | MEMORY | Backend | Service | 장기 기억 (Long-term Memory) |
| `AGENT-WORKER-B-206` | AGENT | WORKER | Backend | Service | 허용된 외부 도구 worker 확장 |
| `AGENT-WORKER-B-207` | AGENT | WORKER | Backend | Service | Code Reasoning Worker 고도화 |
