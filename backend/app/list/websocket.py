...
from app.infra.auth import verify_access_token
...
def _user_id_from_token(token: str | None) -> UUID | None:
    """WebSocket은 Authorization 헤더를 보낼 수 없으므로 query param 토큰을 검증한다."""
    if not token:
        return None
    try:
...
async def websocket_list_progress(
    websocket: WebSocket,
    job_id: str,
